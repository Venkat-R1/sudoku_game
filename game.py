...
class Cube:
    ...
    def draw(self, win):
        ...
        if self.temp != 0 and self.value == 0:
            text = fnt.render(str(self.temp), 1, (128, 128, 128))  # penciling the value
            win.blit(text, (x + 5, y + 5))
        elif not (self.value == 0):
            text = fnt.render(str(self.value), 1, (0, 0, 0))  # entering the value
    ...
    def draw_change(self, win, g=True):
        ...
        win.blit(text, (x + (gap / 2 - text.get_width() / 2), y + (gap / 2 - text.get_height() / 2)))
        if g:
            pygame.draw.rect(win, (0, 255, 0), (x, y, gap, gap), 3)  # green
        else:
            pygame.draw.rect(win, (255, 0, 0), (x, y, gap, gap), 3)  # red
...
def reset(win):
    co = (0, 0, 0)
    color = (255, 255, 255)
    pygame.draw.rect(win, co, [0, 560, 90, 30])
    smallfont = pygame.font.SysFont('Corbel', 20)
    text = smallfont.render('Reset', True, color)
    win.blit(text, (20, 565))


def new(win):
    co = (0, 0, 0)
    color = (255, 255, 255)
    pygame.draw.rect(win, co, [100, 560, 90, 30])
    smallfont = pygame.font.SysFont('Corbel', 20)
    text = smallfont.render('New', True, color)
    win.blit(text, (125, 565))
